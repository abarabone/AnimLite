...
namespace AnimLite.experimental
{
    using AnimLite.Utility;
    using AnimLite.Vmd;
    ...
    public static class HumanBone
    {
        ...
        public static void DeUniformingSkinMeshes(this Animator anim, bool needCloneMesh = true)
        {
            //var a = anim.GetComponent<Animator>();
            //var saveenabled = a.enabled;
            //a.enabled = false;
            var savelocation = moveToZero_RootPositionAndRotation_();


            anim.setHumanBoneTransformRotation();

            var renderers = anim.GetComponentsInChildren<SkinnedMeshRenderer>();

            if (needCloneMesh)
                renderers.rebuildBindposes_WithCloneMesh();
            else
                renderers.rebuildBindposes();


            restore_RootPositionAndRotation_(savelocation);
            //a.enabled = saveenabled;
            return;
        ...
        /// <summary>
        /// ヒューマノイドボーンのＶＲＭ正規化を解除する。
        /// ただし完全ではない。
        /// 下記のような手順で行う。
        /// ・次のボーンワールド位置までの向きを、ＶＲＭ正規化軸に反映する。
        /// ・ＶＲＭ正規化軸は、体幹ならＹ、腕系ならＸ、足系ならＹ、のような感じ。
        /// ・末端などは大体で設定する。
        /// 　- 頭、指先なら１つ前と同じ
        /// 　- 親指は斜め前方にする
        /// 　- 足首ならつま先までの向きのＹを 0 にした向き
        /// 　- 足先 なら足首のＹ軸を 0 にした向き
        /// 　- 肩は正規化のまま
        /// </summary>
        static void setHumanBoneTransformRotation(this Animator anim)
        {
            anim.re_transform_(HumanBodyBones.Hips, Vector3.up);
            anim.re_transform_(HumanBodyBones.Spine, Vector3.up);
            anim.re_transform_(HumanBodyBones.Chest, Vector3.up);
            anim.re_transform_(HumanBodyBones.UpperChest, Vector3.up);
            anim.re_transform_(HumanBodyBones.Neck, Vector3.up);
            anim.re_transform_terminal_ident_(HumanBodyBones.Head);
            ...
            anim.re_transform_(HumanBodyBones.LeftUpperLeg, Vector3.down);
            anim.re_transform_(HumanBodyBones.LeftLowerLeg, Vector3.down);
            anim.re_transform_xz_(HumanBodyBones.LeftFoot, Vector3.forward);
            anim.re_transform_terminal_copyxz_(HumanBodyBones.LeftToes);

            anim.re_transform_(HumanBodyBones.RightUpperLeg, Vector3.down);
            anim.re_transform_(HumanBodyBones.RightLowerLeg, Vector3.down);
            anim.re_transform_xz_(HumanBodyBones.RightFoot, Vector3.forward);
            anim.re_transform_terminal_copyxz_(HumanBodyBones.RightToes);


            //anim.re_transform_(HumanBodyBones.LeftShoulder, Vector3.left);//肩は正規化のままでよいのかも
            anim.re_transform_(HumanBodyBones.LeftUpperArm, Vector3.left);
            anim.re_transform_(HumanBodyBones.LeftLowerArm, Vector3.left);
            //anim.re_transform_terminal_ident_(HumanBodyBones.LeftHand);
            anim.re_transform_(HumanBodyBones.LeftHand, Vector3.left);//中指につなげる

            //anim.re_transform_(HumanBodyBones.RightShoulder, Vector3.right);//肩は正規化のままでよいのかも
            anim.re_transform_(HumanBodyBones.RightUpperArm, Vector3.right);
            anim.re_transform_(HumanBodyBones.RightLowerArm, Vector3.right);
            //anim.re_transform_terminal_ident_(HumanBodyBones.RightHand);
            anim.re_transform_(HumanBodyBones.RightHand, Vector3.right);//中指につなげる


            anim.re_transform_(HumanBodyBones.LeftThumbProximal, math.normalize(Vector3.left + Vector3.forward));
            anim.re_transform_(HumanBodyBones.LeftThumbIntermediate, math.normalize(Vector3.left + Vector3.forward));
            anim.re_transform_terminal_copy_(HumanBodyBones.LeftThumbDistal);

            anim.re_transform_(HumanBodyBones.LeftIndexProximal, Vector3.left);
            anim.re_transform_(HumanBodyBones.LeftIndexIntermediate, Vector3.left);
            anim.re_transform_terminal_copy_(HumanBodyBones.LeftIndexDistal);

            anim.re_transform_(HumanBodyBones.LeftMiddleProximal, Vector3.left);
            anim.re_transform_(HumanBodyBones.LeftMiddleIntermediate, Vector3.left);
            anim.re_transform_terminal_copy_(HumanBodyBones.LeftMiddleDistal);

            anim.re_transform_(HumanBodyBones.LeftRingProximal, Vector3.left);
            anim.re_transform_(HumanBodyBones.LeftRingIntermediate, Vector3.left);
            anim.re_transform_terminal_copy_(HumanBodyBones.LeftRingDistal);

            anim.re_transform_(HumanBodyBones.LeftLittleProximal, Vector3.left);
            anim.re_transform_(HumanBodyBones.LeftLittleIntermediate, Vector3.left);
            anim.re_transform_terminal_copy_(HumanBodyBones.LeftLittleDistal);


            anim.re_transform_(HumanBodyBones.RightThumbProximal, math.normalize(Vector3.right + Vector3.forward));
            anim.re_transform_(HumanBodyBones.RightThumbIntermediate, math.normalize(Vector3.right + Vector3.forward));
            anim.re_transform_terminal_copy_(HumanBodyBones.RightThumbDistal);

            anim.re_transform_(HumanBodyBones.RightIndexProximal, Vector3.right);
            anim.re_transform_(HumanBodyBones.RightIndexIntermediate, Vector3.right);
            anim.re_transform_terminal_copy_(HumanBodyBones.RightIndexDistal);

            anim.re_transform_(HumanBodyBones.RightMiddleProximal, Vector3.right);
            anim.re_transform_(HumanBodyBones.RightMiddleIntermediate, Vector3.right);
            anim.re_transform_terminal_copy_(HumanBodyBones.RightMiddleDistal);

            anim.re_transform_(HumanBodyBones.RightRingProximal, Vector3.right);
            anim.re_transform_(HumanBodyBones.RightRingIntermediate, Vector3.right);
            anim.re_transform_terminal_copy_(HumanBodyBones.RightRingDistal);

            anim.re_transform_(HumanBodyBones.RightLittleProximal, Vector3.right);
            anim.re_transform_(HumanBodyBones.RightLittleIntermediate, Vector3.right);
            anim.re_transform_terminal_copy_(HumanBodyBones.RightLittleDistal);
        }

        static void re_transform_(
            this Animator anim, HumanBodyBones bone, float3 normaldir)
        {
            anim._re_transform_(bone, normaldir, x => math.normalize(x));
        }
        static void re_transform_xz_(this Animator anim, HumanBodyBones bone, float3 normaldir)
        {
            anim._re_transform_(bone, normaldir, x => xz_(x));

            static float3 xz_(float3 v) => math.normalize(new float3(v.x, 0.0f, v.z));
        }
        static void _re_transform_(
            this Animator anim, HumanBodyBones bone, float3 normaldir, Func<float3, float3> f)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
            if (tfCurr == null) return;

            var next = NextBoneDictinary[curr];
            var tfNext = anim.GetBoneTransform(next) ?? anim.GetBoneTransform(NextBoneDictinary[next]);
            if (tfNext == null) return;
            ...
            tfCurr.DetachChildren();

            tfCurr.rotation = wcrot;
            ...
            Transform[] getChildren_() =>
                Enumerable.Range(0, tfCurr.childCount)
                    .Select(x => tfCurr.GetChild(x))
                    .ToArray();

            void re_atach_(Transform[] tfs) =>
                tfs.ForEach(x => x.parent = tfCurr);
        ...
        static void re_transform_terminal_ident_(this Animator anim, HumanBodyBones bone)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
            if (tfCurr == null) return;

            tfCurr.rotation = quaternion.identity;
        }
        static void re_transform_terminal_copy_(this Animator anim, HumanBodyBones bone)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
            if (tfCurr == null) return;

            tfCurr.rotation = tfCurr.parent.rotation;
        }
        static void re_transform_terminal_copyxz_(this Animator anim, HumanBodyBones bone)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
            if (tfCurr == null) return;
            ...
            tfCurr.rotation = quaternion.LookRotation(dirxz, Vector3.up);
        ...
        public static Dictionary<HumanBodyBones, HumanBodyBones> NextBoneDictinary = new()
        {
...
namespace a
{
    public static class a
    {
        ...
        //struct DeUniformingDictionary
        //{

        //    public Dictionary<string, DeUniformingBone> values;


        //    public static DeUniformingDictionary operator + (DeUniformingDictionary dict, DeUniformingBone value)
        //    {
        //        if (value.tf == null) return dict;

        //        dict.values[value.tf.name] = value;
        ...
        public static Dictionary<string, DeUniformingBone> CalculateDeUniformingBoneValues(this Animator anim)
        {
            ...
            anim.re_transform_terminal_ident_(dict, HumanBodyBones.Head);
            ...
            anim.re_transform_terminal_copyxz_(dict, HumanBodyBones.LeftToes);
            ...
            anim.re_transform_terminal_copyxz_(dict, HumanBodyBones.RightToes);


            //anim.re_transform_(dict, HumanBodyBones.LeftShoulder, Vector3.left);//肩は正規化のままでよいのかも
            anim.re_transform_terminal_ident_(dict, HumanBodyBones.LeftShoulder);
            ...
            //anim.re_transform_(dict, HumanBodyBones.RightShoulder, Vector3.right);//肩は正規化のままでよいのかも
            anim.re_transform_terminal_ident_(dict, HumanBodyBones.RightShoulder);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.LeftThumbDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.LeftIndexDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.LeftMiddleDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.LeftRingDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.LeftLittleDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.RightThumbDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.RightIndexDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.RightMiddleDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.RightRingDistal);
            ...
            anim.re_transform_terminal_copy_(dict, HumanBodyBones.RightLittleDistal);
        ...
        static DeUniformingBone add(this Dictionary<string, DeUniformingBone> dict, Transform tf, Vector3 wpos, Quaternion wrot)
        {
            ...
            var mtinv = Matrix4x4.TRS(p.newPosition, p.newRotation, Vector3.one).inverse;
            ...
            return dict[tf.name] = new DeUniformingBone
            {
                ...
                newLocalPosition = mtinv * wpos,
                newLocalRotation = mtinv.rotation * wrot,
            };
        }
            ...
            this Animator anim, Dictionary<string, DeUniformingBone> dict, HumanBodyBones bone, float3 normaldir, Func<float3, float3> f)
        {
            ...
            var next = AnimLite.experimental.HumanBone.NextBoneDictinary[curr];
            var tfNext = anim.GetBoneTransform(next) ?? anim.GetBoneTransform(AnimLite.experimental.HumanBone.NextBoneDictinary[next]);
        ...
        static DeUniformingBone re_transform_terminal_ident_(
        ...
        static DeUniformingBone re_transform_terminal_copy_(
            this Animator anim, Dictionary<string, DeUniformingBone> dict, HumanBodyBones bone)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
        ...
        static DeUniformingBone re_transform_terminal_copyxz_(
            this Animator anim, Dictionary<string, DeUniformingBone> dict, HumanBodyBones bone)
        {
            var curr = bone;
            var tfCurr = anim.GetBoneTransform(curr);
