...
namespace AnimLite.Vmd
{
    ...
    public static class VmdJobExtension
    {
        ...
        public static VmdAnimationJob<TPFinder, TRFinder> create<TPFinder, TRFinder>(
            this Animator anim, JobPlayableStreamingBone bone, TPFinder pkf, TRFinder rkf, StreamingTimer timer)
                where TPFinder : struct, IKeyFinderWithoutProcedure<float4>
                where TRFinder : struct, IKeyFinderWithoutProcedure<quaternion>
        {

            var useIk =
                rkf.Streams.Sections[(int)MmdBodyBones.右足首].length < rkf.Streams.Sections[(int)MmdBodyBones.右足ＩＫ].length
                &&
                rkf.Streams.Sections[(int)MmdBodyBones.左足首].length < rkf.Streams.Sections[(int)MmdBodyBones.左足ＩＫ].length;
            ...
            return new VmdAnimationJob<TPFinder, TRFinder>
            {
                ...
                //useLegPositionIk = rkf.Streams.Sections[(int)MmdBodyBones.左ひざ].length < 3,
                //useFootRotationIk = rkf.Streams.Sections[(int)MmdBodyBones.左足首].length < 3,
                //useLegPositionIk = pkf.Streams.Sections[(int)MmdBodyBones.左足ＩＫ].length > 2,
                //useFootRotationIk = pkf.Streams.Sections[(int)MmdBodyBones.左足ＩＫ].length > 2,
                //useLegPositionIk = rkf.Streams.Sections[(int)MmdBodyBones.左ひざ].length < pkf.Streams.Sections[(int)MmdBodyBones.左足ＩＫ].length,
                //useFootRotationIk = rkf.Streams.Sections[(int)MmdBodyBones.左足首].length < rkf.Streams.Sections[(int)MmdBodyBones.左足ＩＫ].length,
                useLegPositionIk = useIk,
                useFootRotationIk = useIk,

                body = anim.ToVmdBodyMotionOperator<JobPlayableStreamingBone, TfHandle>(bone),

                foot = anim.ToFootIkOperator<JobPlayableStreamingBone, TfHandle>(bone),
            };
        }
    }
        ...
        where TRFinder : struct, IKeyFinderWithoutProcedure<quaternion>
    {
        ...
        //public VmdBodyMotionJobPlayableOperator body;
        public VmdBodyMotionOperator<JobPlayableStreamingBone, TfHandle> body;
        ...
        // つまり、相対的移動量のみがデータとなっており、root 位置は足元のまま移動させられる、と考える。
